...
from tools.utils import validate_city_format, TransportationError, to_string

data_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../database/googleDistanceMatrix/distance.csv"))
valid_transportation_modes = ['self-driving', 'taxi']

def validate_transportation_mode(mode):
    if mode not in valid_transportation_modes:
        raise TransportationError(mode)
    return True
...
class GoogleDistanceMatrix:
    ...
    def run(self, origin, destination, mode='self-driving'):
    ...
    def query(self, origin, destination, mode='self-driving') -> str:
        if validate_city_format([origin, destination]) and validate_transportation_mode(mode):
            return self.run(origin, destination, mode)
        
    def search(self, origin, destination, mode='self-driving') -> str:
        return to_string(self.query(origin, destination, mode))
    
    def view(self, origin, destination, mode='self-driving') -> str:
        return to_string(self.query(origin, destination, mode), verbose=False)
