from utils import get_valid_name_city,extract_before_parenthesis,extract_numbers_from_filenames
        ...
        if unit['transportation'] and  unit['transportation'] != '-':
            ...
            if org_city == None or dest_city == None:
                ...
                if 'flight number' in value.lower():
                    res = flight.data[flight.data['Flight Number'] == re.search(r"[a-zA-Z]\d+", value).group()]
                    if len(res) > 0:
    ...
    if question['cuisine']:
        for i in range(min(question['days'],len(tested_data))):
            ...
            if unit['breakfast'] and unit['breakfast'] != '-':
                ...
                res = restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (restaurants.data['City'] == city)]
                if len(res) > 0:       
                    for cuisine in question['cuisine']:
        ...
        if len(cuisine_set) == len(question['cuisine']):
            return True, None
        else:
            ...
            # return False, f"The cuisine should be {question['cuisine']}."
    else:
        return None,None
    

def is_valid_transportation(question, tested_data):
    if len(question['transportation']) == 0:
        return None,None
    for i in range(min(question['days'],len(tested_data))):
        unit = tested_data[i]
        if unit['transportation'] and unit['transportation'] != '-':
            value = unit['transportation']
            if question['transportation'] == 'no flight' and 'Flight' in value:
                return False, f"The transportation should not be {question['transportation']}."
            elif question['transportation'] == 'no self-driving' and 'Self-driving'  in value:
                return False, f"The transportation should not be {question['transportation']}."
            
    return True, None


def is_valid_housing(question, tested_data):
    if len(question['housing']) == 0:
        return None, None
    for i in range(min(question['days'],len(tested_data))):
        unit = tested_data[i]
        if unit['accommodation'] and unit['accommodation'] != '-':
            ...
            res = accommodation.data[(accommodation.data['NAME'].astype(str).str.contains(re.escape(name))) & (accommodation.data['city'] == city)]
            if len(res) > 0:
                if 'not shared room' in question['housing'] and res['room type'].values[0] == 'Shared room':
                    return False, f"The room type should be 'not shared room'."
                # "shared room", "not shared room", "private room", "entire room"
                elif 'shared room' in question['housing'] and res['room type'].values[0] != 'Shared room':
                    return False, f"The room type should be 'shared room'."

                elif 'private room' in question['housing'] and res['room type'].values[0] != 'Private room':
                    return False, f"The room type should be 'private room'."

                elif 'entire room' in question['housing'] and res['room type'].values[0] != 'Entire home/apt':
                    return False, f"The room type should be 'entire room'."
                
                if 'smoking' in question['housing'] and 'No smoking' in str(res['house_rules'].values[0]):
                    return False, f"The house rule should be 'smoking'."
                
                elif 'parties' in question['housing'] and 'No parties' in str(res['house_rules'].values[0]):
                    return False, f"The house rule should be 'parties'."
                
                elif 'children under 10' in question['housing'] and 'No children under 10' in str(res['house_rules'].values[0]):
                    return False, f"The house rule should be 'children under 10'."
                
                elif 'visitors' in question['housing'] and 'No visitors' in str(res['house_rules'].values[0]):
                    return False, f"The house rule should be 'visitors'."
                
                elif 'pets' in question['housing'] and 'No pets' in str(res['house_rules'].values[0]):
                    return False, f"The house rule should be 'pets'."
...
def evaluation(query_data, tested_data):
    ...
    return_info['valid_housing'] = is_valid_housing(query_data, tested_data)
